use rand::Rng;
use std::{collections::HashMap, sync::mpsc::SyncSender, thread, time::Duration};
...
impl ChannelMockTransport {
    pub fn new(network_delay_range: std::ops::RangeInclusive<u64>) -> Self {
        ChannelMockTransport { senders: HashMap::new(), network_delay_range }
    }
...
impl Transport for ChannelMockTransport {
    fn send(&self, target: NodeID, message: NetworkMessage) {
        ...
        let delay =
            Duration::from_millis(rand::thread_rng().gen_range(self.network_delay_range.clone()));
        thread::spawn(move || {
            thread::sleep(delay);
